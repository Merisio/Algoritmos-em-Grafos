...
// Conta quantos vertices tem o grafo
...
void busca_largura(Grafo *g, int ini){
    int i, vertice, num_vertices, *visitado;
    Fila *fila = criar_fila();

    num_vertices = numVertices(g);
    visitado = (int*)calloc(num_vertices, sizeof(int)); // Marcando todos do vetor com 0, pois nenhum foi visitado ainda

    NoGrafo* verticeInicial = encontra_vertice(g, ini);

    if (verticeInicial == NULL){ // Tratamento para caso o vertice nao exista no grafo
        printf("Vertice inicial não encontrado no grafo.\n");
        liberar_fila(fila);
        free(visitado);
        return;
    }

    visitado[ini] = 1; // Marca o vertice inicial como visitado
    enfileirar(fila, ini);

    while (!fila_vazia(fila)) { // Enquanto a fila não estiver vazia
        vertice = desenfileirar(fila); // Pega o primeiro da fila
        printf("%d ", vertice);

        NoGrafo* vert = encontra_vertice(g, vertice);
        NoAdj* adj = vert->lista->head;

        while (adj != NULL){
            if (visitado[adj->item] == 0){
                enfileirar(fila, adj->item); // Enfileira o vertice na fila
                visitado[adj->item] = 1; // Marca como visitado no vetor
            }
            adj = adj->next;
        }
    }
